...
tab1, tab2, tab3 = st.tabs(["🧮 คำนวณ & บันทึกออเดอร์", "📜 ประวัติ & แก้ไขออเดอร์", "📊 สรุปภาพรวมพอร์ต"])
...
with tab1:
    ...
    with col_input:
        ...
        symbol = st.selectbox("สินทรัพย์", ["XAUUSD (ทองคำ)", "Forex (General)"])
        ...
        if calc_mode == "กรอกราคา SL/TP ตรงๆ":
            ...
            sl_distance = st.number_input("ระยะ SL (ดอลลาร์/Oz)", min_value=0.1, value=3.0, step=0.5)
    ...
    # คำนวณผลลัพธ์
    multiplier = 100.0 if "XAUUSD" in symbol else 100000.0  # สำหรับ XAUUSD 0.01 lot = $1/point
    if side == "BUY":
...
with tab2:
    ...
    if not st.session_state.trades:
        ...
        st.divider()
        st.subheader("✏️ แก้ไข SL/TP หรือปิดสถานะออเดอร์")
        
        trade_ids = [t["id"] for t in st.session_state.trades]
        selected_id = st.selectbox("เลือก ID ออเดอร์ที่ต้องการแก้ไข", trade_ids)
        
        # ดึงออเดอร์ที่เลือกมาแก้ไข
        ...
        with edit_c2:
            new_status = st.selectbox("สถานะออเดอร์", ["Active (ถืออยู่)", "TP Hit (ชนะ)", "SL Hit (แพ้)", "Closed Manual (ปิดมือ)"], index=["Active (ถืออยู่)", "TP Hit (ชนะ)", "SL Hit (แพ้)", "Closed Manual (ปิดมือ)"].index(current_trade["status"]))
            
            # กำหนดกำไร/ขาดทุนจริง
        ...
        with edit_c3:
            ...
            st.write(" ")
            if st.button("🔄 อัปเดตข้อมูลออเดอร์", type="primary"):
                # คำนวณค่า Risk/Reward ใหม่กรณีแก้ไข SL/TP
...
# ==========================================
with tab3:
    st.header("📊 สรุปผลการเทรดรวม (Performance)")
